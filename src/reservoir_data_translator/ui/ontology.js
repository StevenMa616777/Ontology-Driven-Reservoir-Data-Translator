...
const palette = { reservoir_simulation: "#263b36", physical: "#397da5", rock: "#ad694b", fluid: "#267c71", scal: "#988431", well: "#526ea6", schedule: "#8b5f91", condition: "#b36b79" };
const state = { data: null, scope: "", selected: null, focus: null, domains: new Set(), relations: new Set(), query: "", transform: { x: 0, y: 0, scale: 1 }, positions: new Map() };
const el = Object.fromEntries(["explorer-shell","ontology-version","concept-search","search-results","domain-filters","scope-filter","relation-filters","concept-tree","graph-title","graph-count","graph-stage","ontology-graph","graph-viewport","graph-edges","graph-nodes","graph-status","concept-detail","reset-view","fit-graph","focus-neighborhood"].map(id => [id.replaceAll("-", "_"), document.getElementById(id)]));
...
function inScope(node) { return !state.scope || (node.scope_bindings || []).some(b => b.context.scope === state.scope); }
function visibleNodes() { const focused = state.focus ? neighborhood(state.focus) : null; return state.data.nodes.filter(node => inScope(node) && (state.domains.has(node.domain) || node.id === "reservoir_simulation") && (!focused || focused.has(node.id))); }
function visibleEdges(nodes) { const ids = new Set(nodes.map(node => node.id)); return state.data.edges.filter(edge => ids.has(edge.source) && ids.has(edge.target) && state.relations.has(edge.type) && (!state.scope || !edge.context || edge.context.scope === state.scope)); }
function nodeMatches(node) { const q = state.query.trim().toLocaleLowerCase(); return !q || [node.id,node.label,node.description,...node.aliases,...(node.scope_bindings || []).flatMap(b => [JSON.stringify(b.context),...b.aliases])].join(" ").toLocaleLowerCase().includes(q); }

function buildControls() {
  el.scope_filter.innerHTML += state.data.scopes.map(scope => `<option value="${escapeHtml(scope.id)}">${escapeHtml(scope.name)}</option>`).join("");
  el.scope_filter.addEventListener("change", event => { state.scope = event.target.value; state.focus = null; renderAll(); fitGraph(); });
...
function renderTree() {
  ...
  const rows = [];
  function walk(parent, depth) { (children.get(parent) || []).sort((a,b) => a.id.localeCompare(b.id)).forEach(node => { if (inScope(node) && (state.domains.has(node.domain) || !node.parent) && nodeMatches(node)) rows.push(`<button class="tree-node ${node.id === state.selected ? "is-selected" : ""}" data-id="${escapeHtml(node.id)}" style="padding-left:${8 + depth * 12}px" title="${escapeHtml(node.id)}">${escapeHtml(node.label)}</button>`); walk(node.id, depth + 1); }); }
  if (state.query) state.data.nodes.filter(node => inScope(node) && (state.domains.has(node.domain) || !node.parent) && nodeMatches(node)).forEach(node => rows.push(`<button class="tree-node ${node.id === state.selected ? "is-selected" : ""}" data-id="${escapeHtml(node.id)}" style="padding-left:8px">${escapeHtml(node.label)}</button>`)); else walk("", 0);
...
function layout(nodes) {
  if (state.scope) {
    const rows = [[], [], []];
    nodes.forEach(node => {
      const roles = (node.scope_bindings || []).filter(b => b.context.scope === state.scope).map(b => b.context.role);
      const row = roles.some(role => ["model", "entity", "type"].includes(role)) ? 0 : roles.some(role => ["coordinate", "reference"].includes(role)) ? 1 : 2;
      rows[row].push(node);
    });
    const width = Math.max(...rows.map(row => row.length)) * 170;
    rows.forEach((row, level) => row.sort((a,b) => a.id.localeCompare(b.id)).forEach((node, index) => state.positions.set(node.id, { x: 100 + (width - row.length * 170) / 2 + index * 170, y: 90 + level * 145 })));
    return;
  }
...
function renderRelationRows(node) { const rows = []; Object.entries(node.relationships).forEach(([type, targets]) => targets.forEach(target => rows.push([type, target]))); node.incoming_relationships.filter(rel => !state.scope || !rel.context || rel.context.scope === state.scope).forEach(rel => rows.push([`← ${rel.type}${rel.context ? " (" + Object.values(rel.context).join(" · ") + ")" : ""}`, rel.source])); return rows.length ? rows.map(([type,target]) => `<div class="relation-row"><b>${escapeHtml(type)}</b><button type="button" data-id="${escapeHtml(target)}">${escapeHtml(target)}</button></div>`).join("") : '<span class="search-note">无声明关系</span>'; }
function renderBindings(node) { return (node.scope_bindings || []).filter(b => !state.scope || b.context.scope === state.scope).map(binding => `<div class="detail-section"><strong>${escapeHtml(Object.values(binding.context).join(" · "))}</strong><p>${escapeHtml(binding.aliases.join(" / "))}</p><div class="relation-list">${Object.entries(binding.relationships).map(([relation, targets]) => `<div>${escapeHtml(relation)} → ${escapeHtml(targets.join(", "))}</div>`).join("")}</div>${binding.canonical_resolution ? `<span>Canonical 解析规则</span><code class="detail-source">${escapeHtml(binding.canonical_resolution.path_template)}</code>` : ""}</div>`).join("") || '<span class="search-note">未配置任务作用域</span>'; }
function renderDetail(node) { const constraints = Object.keys(node.constraints).length ? Object.entries(node.constraints).map(([k,v]) => `<span class="chip">${escapeHtml(k)}: ${escapeHtml(v)}</span>`).join("") : '<span class="search-note">无</span>'; el.concept_detail.innerHTML = `<div class="detail-header"><span class="detail-domain">${escapeHtml(node.domain)} · ${escapeHtml(node.status)}</span><h2>${escapeHtml(node.label)}</h2><div class="detail-id">${escapeHtml(node.id)}</div><p class="detail-description">${escapeHtml(node.description)}</p></div><div class="detail-grid"><div><span>Value type</span><strong>${escapeHtml(node.value_type)}</strong></div><div><span>Parent</span><strong>${escapeHtml(node.parent)}</strong></div><div><span>Dimension</span><strong>${escapeHtml(node.dimension)}</strong></div><div><span>Canonical unit</span><strong>${escapeHtml(node.canonical_unit)}</strong></div></div><div class="detail-section"><span>Aliases</span><div class="chip-list">${node.aliases.map(a => `<span class="chip">${escapeHtml(a)}</span>`).join("") || '<span class="search-note">无</span>'}</div></div><div class="detail-section"><span>Constraints</span><div class="chip-list">${constraints}</div></div><div class="detail-section"><span>Outgoing & Incoming relationships</span><div class="relation-list">${renderRelationRows(node)}</div></div><div class="detail-section"><span>作用域参与方式 · Scope bindings</span>${renderBindings(node)}</div><div class="detail-section"><span>Source</span><div class="detail-source">${escapeHtml(node.source_file)}</div></div>`; }
...
function renderAll() { renderSearch(); renderTree(); renderGraph(); if (state.selected) renderDetail(byId(state.selected)); }
...
function bindEvents() {
  ...
  el.reset_view.addEventListener("click", () => { state.focus = null; state.scope = ""; el.scope_filter.value = ""; state.query = ""; el.concept_search.value = ""; renderAll(); fitGraph(); });
